...
namespace UnityTK
{
    ...
    public class StaticBatching : MonoBehaviour
    {
        ...
        private struct BatchingCommandInsert
        {
            ...
            /// <summary>
            /// The visual representation gameobject.
            /// </summary>
        ...
        /// </summary>
        private Dictionary<GameObject, StaticVisualRepresentationCache> visCache = new Dictionary<GameObject, StaticVisualRepresentationCache>();
        ...
        /// </summary>
        /// <param name="visualRepresentation">The gameobject to retrieve the cache for.</param>
        /// <returns>The representation cache information.</returns>
        private StaticVisualRepresentationCache GetVisCache(GameObject visualRepresentation)
        {
            StaticVisualRepresentationCache cache;
            if (!this.visCache.TryGetValue(visualRepresentation, out cache))
                cache = CreateVisCache(visualRepresentation);
        ...
        /// </summary>
        private StaticVisualRepresentationCache CreateVisCache(GameObject visualRepresentation)
        {
            // Get components
            var mr = visualRepresentation.GetComponent<MeshRenderer>();
            var mf = visualRepresentation.GetComponent<MeshFilter>();

            // Validate
            if (ReferenceEquals(mr, null))
                throw new System.InvalidOperationException("Tried creating visualization cache for gameobject without mesh renderer!");

            if (ReferenceEquals(mf, null))
                throw new System.InvalidOperationException("Tried creating visualization cache for gameobject without mesh filter!");

            var mesh = mf.sharedMesh;
            var material = mr.sharedMaterial;
            var cache = new StaticVisualRepresentationCache(material, mesh);

            this.visCache.Add(visualRepresentation, cache);
            return cache;
        }
        ...
        public void InsertVisualRepresentation(GameObject visualRepresentation, Matrix4x4 transform, object owner)
        {
            ...
            foreach (var mr in meshRenderers)
            {
                ...
                this.insertQueue.Add(new BatchingCommandInsert()
                {
                    ...
                    visualRepresentation = mr.gameObject,
                    owner = owner,
        ...
        private struct TransformJob : IJobParallelFor
        {
            ...
            public void Execute(int index)
            {
                ...
                var visCache = StaticBatching.instance.GetVisCache(meshInstance.visualRepresentation);
        ...
        private void QueueRebuild(StaticBatchingGroupKey group)
        {
            ...
            {
                var cache = GetVisCache(instance.visualRepresentation);
            ...
            this.runningJobs.Add(this.runningJobDataCounter++, jobData);
        ...
        private void RebuildUpdate()
        {
            ...
            StaticBatchingGroupKey groupKey = jobData.groupKey;
            ...
            // Flush
            jobData.Flush(m);
            this.runningJobs.Remove(dataId);
        }
        ...
        private void InsertCommand(BatchingCommandInsert command, HashSet<StaticBatchingGroupKey> rebuilds)
        {
            ...
            this.meshInstances.GetOrCreate(groupKey).Add(new StaticMeshInstance()
            {
                ...
                visualRepresentation = command.visualRepresentation
            });
